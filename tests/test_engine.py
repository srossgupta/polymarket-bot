...
def _make_market(end_minutes=2) -> Market:
    return Market(market_id="test_mkt", question="Test market?",
                  end_time=datetime.now(timezone.utc) + timedelta(minutes=end_minutes),
                  volume_usd=200_000, category="sports",
...
class TestMonitorMarket:
    def test_enters_and_wins(self):
        ...
        prices = iter([PricePoint(ts=datetime.now(timezone.utc), yes=0.96, no=0.04),
        ...
        stats = monitor_market_until_close(mock_client, portfolio, market, cfg,
                                            now_fn=now_fn, sleeper=lambda x: None)
        assert stats["entered"]

    def test_skips_low_volume(self):
        market = _make_market()
        market.volume_usd = 50_000
        stats = monitor_market_until_close(MagicMock(), PaperPortfolio(BotConfig()), market,
                                            BotConfig(), sleeper=lambda x: None)
    ...
    def test_stop_loss_triggers(self):
        ...
        stats = monitor_market_until_close(mock_client, portfolio, market, cfg,
                                            now_fn=now_fn, sleeper=lambda x: None)
        assert stats["entered"] and stats["stopped"]


class TestBacktestIntegration:
    def test_backtest_runs(self, tmp_path, monkeypatch):
        monkeypatch.setattr("polymarket_bot.core.config.DATA_DIR", str(tmp_path))
        monkeypatch.setattr("polymarket_bot.core.config.ADAPTIVE_STATE_FILE", str(tmp_path / "adaptive.json"))
        result = run_backtest()
        assert "source" in result and result["result"]["trades"] >= 0
