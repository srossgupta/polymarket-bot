...
from .models import (Market, PerformanceSnapshot, Position, PricePoint,
                     PriceSeries, Side, TradeEvent, TradeType)
from .storage import append_trade, save_performance
from .strategy import compute_position_size, volatility_adjusted_stop
...
class PaperPortfolio:
    ...
    def open_position(
        ...
        # Position sizing: use Kelly if available, else max per market
        if kelly_fraction > 0:
            allocation = compute_position_size(
                price, kelly_fraction,
                self.cfg.strategy.max_dollars_per_market, self.cash)
        else:
        ...
        stop_price = volatility_adjusted_stop(
            price, volatility, self.cfg.strategy.stop_loss_cents)
        ...
        event = TradeEvent(
            ...
            category=market.category,
            side=side.value,
            event_type=TradeType.BUY_LIMIT.value,
            ts=pos.entry_ts,
            price=price,
            size_dollars=allocation,
            shares=shares,
            reason=reason,
            volume_at_entry=market.volume_usd,
    ...
    def mark_to_market(self, prices: dict[str, PricePoint]) -> float:
        ...
        for mid, pos in self.open_positions.items():
            ...
            if point:
                ...
                value += pos.size_dollars  # assume par
        return value
    ...
    def sharpe_estimate(self) -> float:
        """Simplified Sharpe: mean(pnl) / std(pnl) * sqrt(trades_per_day_estimate)."""
    ...
    def take_performance_snapshot(self, prices: dict[str, PricePoint] | None = None) -> PerformanceSnapshot:
        ...
        # Category analysis
        from .storage import get_category_performance
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "total_value": round(self.cash + sum(
                p.size_dollars for p in self.open_positions.values()), 2),
            "open_positions": len(self.open_positions),
            "total_trades": self.total_trades,
            "wins": self.wins,
            "losses": self.losses,
